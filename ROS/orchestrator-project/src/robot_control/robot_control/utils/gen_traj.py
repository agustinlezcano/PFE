...
class TrajectoryPlanner:
    ...
    def plan(self, waypoints, plot=False):
        """
        ...
        for k in range(N):
            T_obj = np.array([
                [1, 0, 0, x[k]],
                [0, 1, 0, y[k]],
                [0, 0, 1, z[k] + self.kinematics.config.tool_z],
                [0, 0, 0, 1]
            ])
            q_eslabon[k, :] = self.kinematics.inverse_kinematics(T_obj)
...
def generate_trajectory():
    ...
    return planner.plan(Pxyz, plot=True)
